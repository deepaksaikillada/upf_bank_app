...
conn_str = (
    ...
    'SERVER=fractalbankserver.database.windows.net,1433;'
    'DATABASE=upfdb;'
    'UID=bindu;'
    'PWD=Password123*;'
    'Encrypt=yes;'
...
account_key = "X1yQxoCAHwSbZa8WOR42GVZ8kdPaEmEUnBHOvfuEz6tEH1zbyzjCB69+PyLwq0hv1bmBf4mQj0IY+AStSMRf+g=="
...
def process_kyc():
    if 'username' in session:
        if 'photo' in request.files:
            ...
            upload_to_azure(photo, filename_with_id)

            # Store the association between the user and the uploaded file in the database
            store_kyc_association(session['username'], filename_with_id)

            flash('KYC form uploaded successfully!', 'success')
        else:
            flash('No KYC form submitted', 'error')
